...
class Node:
    ...
    def __init__(self, data, target, max_depth, target_num):
        self.data = data
        self.target = target
        self.left = None
        self.right = None
    ...
    def split(self, depth):
        ...
        self.left = Node(
            self.data[left_num], self.target[left_num], self.max_depth, self.K)
        self.right = Node(
            self.data[right_num], self.target[right_num], self.max_depth, self.K)
...
class DecisionTree():
    ...
    def fit(self, train_data, target):
        self.tree = Node(train_data, target, self.max_depth, self.K)
        self.tree.split(0)
...
def main():
    ...
    d_tree = DecisionTree(max_depth=4, target_num=len(set(target)))
